...
$.tuanIds = [];
$.whiteName = ['782108446_m', 'jd_iQBiUmkQxfRF']
$.appId = 10001;
if ($.isNode()) {
    ...
    if (cookiesArr[i]) {
      ...
      if ((cookiesArr && cookiesArr.length >= ($.tuanNum || 5)) && $.canHelp) {
        ...
        for (let item of $.tuanIds) {
            if ($.whiteName.indexOf(item.name) >= 0) {
                console.log('白名单参团', item.name)
                console.log(`\n${$.UserName} 去参加团 ${item.id}`);
                  if (!$.canHelp) break;
                  await JoinTuan(item.id);
                  await $.wait(1000);
            }
