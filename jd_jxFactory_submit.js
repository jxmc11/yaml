...
$.userTuanInfo = {};
$.appId = 10001;
$.activeId = 'laD7IwPwDF1-Te-MvbW9Iw=='
...
function getTuanId() {
  return new Promise(async resolve => {
    $.get(taskUrl('tuan/QueryActiveConfig', `activeId=${$.activeId}`, `_time,activeId,tuanId`), async (err, resp, data) => {
      try {
...
function getTuanInfo(body) {
  return new Promise(async resolve => {
    $.get(taskUrl('tuan/QueryTuan', `activeId=${$.activeId}&${body}`, `_time,activeId,tuanId`), async (err, resp, data) => {
      try {
...
function submitTuanId(userName) {
  return new Promise(resolve => {
    ...
    $.post(
      ...
      (err, resp, _data) => {
        console.log('111111111111', _data)
        try {
          const { data = {} } = JSON.parse(_data);
          $.log(`\n团码提交成功：${data.value}\n${$.showLog ? _data : ''}`);
          if (data.value) {
...
function createTuan() {
  return new Promise(async resolve => {
    $.get(
      taskTuanUrl('tuan/CreateTuan', `activeId=${$.activeId}&isOpenApp=1`, '_time,activeId,isOpenApp'),
  ...
  return new Promise(async resolve => {
    $.get({ url: 'https://api.ninesix.cc/api/jx-factory-tuan' }, (err, resp, _data) => {
      try {
        ...
        $.log(`\n${data.value}\n${$.showLog ? _data : ''}`);
        $.get(
          taskTuanUrl('tuan/JoinTuan', `activeId=${$.activeId}&tuanId=${data.value}`, '_time,activeId,tuanId'),
          async (err, resp, data) => {
            try {
              const { msg } = JSON.parse(data);
              $.log(
...
function awardTuan() {
  return new Promise(async resolve => {
    ...
    $.get(
      taskTuanUrl('tuan/Award', `activeId=${$.activeId}&tuanId=${$.userTuanInfo.tuanId}`, '_time,activeId,tuanId'),
      async (err, resp, data) => {
        try {
